// Supabase-backed CommunityPost adapter
...
class CommunityPostAdapter {
  ...
  async save() {
    ...
    const row = {
      ...
      pinned: this.is_pinned ?? false,
      created_by: this.author_id ? String(this.author_id) : null,
  ...
  static async findById(id) {
    ...
    return data ? new CommunityPostAdapter({ ...data, _id: data.id }) : null;
  }
    ...
    const chain = {
      ...
      async then(resolve, reject) {
        try {
          ...
          if (error) throw error;
          resolve((data || []).map(d => new CommunityPostAdapter({ ...d, _id: d.id })));
  ...
  static async findByIdAndUpdate(id, update = {}, options = {}) {
    const current = await CommunityPostAdapter.findById(id);
    ...
    if (error) throw error;
    const result = options && options.new === false ? current : new CommunityPostAdapter({ ...data, _id: data.id });
  ...
  static async findByIdAndDelete(id) {
    const existing = await CommunityPostAdapter.findById(id);
...
module.exports = CommunityPostAdapter;
