// Supabase-backed CommunityMember adapter
...
class CommunityMemberAdapter {
  ...
  async save() {
    ...
    const row = {
      ...
      contribution_hours: this.contribution_hours || 0,
  ...
  static async findById(id) {
    ...
    return data ? new CommunityMemberAdapter({ ...data, _id: data.id }) : null;
  }
    ...
    const chain = {
      ...
      async then(resolve, reject) {
        try {
          ...
          if (error) throw error;
          resolve((data || []).map(d => new CommunityMemberAdapter({ ...d, _id: d.id })));
  ...
  static async findByIdAndUpdate(id, update = {}, options = {}) {
    const current = await CommunityMemberAdapter.findById(id);
    ...
    if (error) throw error;
    const result = options && options.new === false ? current : new CommunityMemberAdapter({ ...data, _id: data.id });
  ...
  static async findByIdAndDelete(id) {
    const existing = await CommunityMemberAdapter.findById(id);
...
module.exports = CommunityMemberAdapter;
